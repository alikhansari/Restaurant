...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
    ...
    $get_total_rows = $order->GetOrderType1(); //hold total records in variable
    //break records into pages
    $total_pages = ceil($get_total_rows/$item_per_page);
    //get starting position to fetch the records
    $page_position = (($page_number-1) * $item_per_page);
    //Limit our results within a specified range.
    $results = $db->query("SELECT * FROM tbl_order WHERE status = 1 ORDER BY id DESC LIMIT $page_position,$item_per_page");
    ...
    ?>
        <table width="100%">
            <thead>
            <tr><td>#</td><td><?php echo $str['username']?></td><td><?php echo $str['food_name'];?></td><td><?php echo $str['number'] ?></td><td><?php echo $str['more']; ?></td></tr>
            </thead>
            <tbody>
            <?php
            foreach($re as $row){ ?>
